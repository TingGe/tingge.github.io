...
(function (window) {
  ...
  (function(){
    ...
    countryColors.append("h4").text("\u88AB\u653B\u51FB\u56FD\u5BB6");
...
// Keep track of the rate
  ...
  function start(loc, stime, etime) {
    var pauser = {
    ...
    var reconnect = function () {
      ...
      //d3.select("#events-data").selectAll("tr.row").remove();
      //d3.select("#events-data").append("tr").attr('class', 'row').html("<td colspan='7'><span style='display: inline-block; height: 152px; vertical-align: middle;'></span></td>");

      if (wsDiscTime > settings.wsTimeout) {
        showMessage("We are having difficulties in the Server connectivity. We will continue trying...");
        wsDiscTime = 0;
      }

      setTimeout(function () {
        start(loc, stime, etime);
      }, interval);
    };
  }
