...
| 产品                | 特点                                       |
| ----------------- | ---------------------------------------- |
| Redash            | 提供了基于 web 的数据库查询和数据可视化功能。 redash 提供了这么一种简单便捷的方式将这些工作标准化：在 web 页面上输入 sql 语句，使用查询返回的结果构造数据可视化图表，用户可以将这些图表结果拖动构造成自己需要的数据面板，在很大程度上可以缓解数据运维人员对前端开发人员的需求。 |
