function Mover(a, b) {
  if (this.created = a.created,
      this.x = a.x,
      this.y = a.y,
      this.offset = a.offset,
      this.destination = a.destination,
      this.layer = b.el,
      this.color = "hsl(" + a.port % 360 + ", 100%, 70%)",
      this.hit = a.onMoverHit,
    this.x === this.destination.x && this.y === this.destination.y
  )
    return void thisL.hit();
  var clippath = "clippath-" + Math.random();
  this.clipPath = b._defs.append("clipPath").attr("id", clippath),
    ...
    this.obj = this.layer.append("polygon").datum(this).classed("animating", !0).attr("points", trail.points).attr("clip-path", "url(#" + clippath + ")").attr("fill", this.color);
  var destination = this.destination;
  return this.dx = destination.lx - this.x,
    this.dy = destination.ly - this.y,
    this.distance = Math.sqrt(this.dx * this.dx + this.dy * this.dy),
    this.update(),
    a.zerg && this.distance > 3 * trail.height ? this.startCurve() : this.start(), this
}
...
ipv.hash = {
  ...
  ipv.stream.http = function (stream, callback) {
    callback = callback || function () {
      };
    $.ajax({
      url: stream.host,
      data: 'stime=' + Math.round(new Date().setMinutes(new Date().getMinutes() - 5) / 1000) + '&etime=' + Math.round(new Date().setMinutes(new Date().getMinutes()) / 1000),
      type: 'get',
    ...
    function reconnect() {
      var d = this;
      setTimeout(function () {
        c = ipv.stream.http(stream, callback), d.timeout *= 2
      }, this.timeout)
    }
  },
...
Mover.prototype.startCurve = function () {
  ...
  Mover.prototype.start = function () {
    ...
    if (e > d)var g = 1, h = d, i = 3000; else var g = e / d, h = e, i = 750;
    ...
    return a.layout = function (c) {
      ...
      a.update = function (b) {
        ...
        for (var h = 0; h < b.datatables.length; h++) {
          var i = b.datatables[h],
            j = c.datatables[i.type].selectAll(".row").data(i.data),
            k = (j.enter().append("tr").classed("row", !0),
              j.selectAll("td").data(cellfmt[i.type]));
          k.html(String),
            k.enter().append("td").attr("class", function (a, b) {
...
// 控制面板。minibar、ui-clock、minimizebutton
  ipv.ui.widgets = {
    minibar: function () {
      function a(a) {
        a.each(function () {
    ...
    minimizebutton: function () {
      var a = !1,
        b = ["+", "_"],
        c = this.append("div").classed("minimize-button minimize", !0).text("_"),
        d = c.style("height");
...
// 单元数据格式化
  ipv.ui.cellfmt = {
    ...
    events: function (a) {
      var b = a.serviceObj;
      return [a.timestamp, a.org, a.ip, a.source, a.target, '<span style="color: ' + b.color + '">' + a.svc + "</span>", b.port]
    }
  },
  ...
  function (window, $) {
    ...
    // 获取攻击数据
    function main(a, json_config, nmap_services_1500, country_codes, json_layout, particleHidden) {
      // UI-Clock
      function h(a) {
        var uiClock = document.querySelector("#ui-clock svg");
        uiClock.appendChild(a);
        ...
        d3.select("#svg-toggle-info").on("mousedown", function () {
          ...
          d3.select("#infobox .close").on("mousedown", function () {
            d3.select("#infobox").transition().duration(1e3).style({left: "100%", display: "none"})
          }),
        ...
        d3.select(".minimize-button").on("mousedown.toggle", function () {
          d ? (a.setAttribute("transform", "translate(" + [J.center.x, J.center.y] + ")"), uiClock.appendChild(a)) : (a.setAttribute("transform", "translate(40, 40)"), c.appendChild(a)), d = !d
        })
      }

      if (a)throw a;
      if (M = particleHidden, countryCodes = country_codes, g(json_config.map), !k.embed)
        if (J = ipv.ui().layout(json_layout), K = d3.select("body").append("div").attr("id", "ui").call(J), document.getElementById("ui").appendChild(document.getElementById("infobox")), isInline) {
          var j = d3.select(document.getElementById("chevronHiddenDiv")).select("g").node();
          h(j)
        } else d3.select("#controls #logo").attr("src", json_config.ui.logo), d3.xml("assets/chevron.svg", "image/svg+xml", function (a) {
          var b = d3.select(a.documentElement).select("g").node();
          h(b)
        });
      l = ipv.stream.http(json_config.stream, render)
    }
    ...
    /*
     * 获取http attack数据后，执行渲染
     */
    function render(b) {
      var temp = [], data = eval(b);
      if (!data) {
        return false;
      }
      temp = _.sortBy(data, function (item) {
        return -item.time;
      });
      for (var i = 0, len = temp.length; i < len; i++) {
        var datum = temp[i];
        datum.zerg = "";
        setTimeout(renderCell(datum), 3e6);
      }
    }

    function renderCell(datum) {
      if (datum, "O1" !== datum.country && "O1" !== datum.country2) {
        var c = {
            source: {
              key: (lat = datum.latitude) + "_" + (lng = datum.longitude),
              latLng: [+lat, +lng],
              country: datum.country,
              locString: countryCodes[datum.country] + ", " + (datum.city ? datum.city : "")
            },
            target: {
              key: (lat = datum.latitude2) + "_" + (lng = datum.longitude2),
              latLng: [+lat, +lng],
              country: datum.country2,
              locString: countryCodes[datum.country2] + ", " + (datum.city2 ? datum.city2 : "")
            }
          },
          d = +datum.dport,
          svc = datum.svc,
          count = 0,
          e = "hsl(" + +d % 360 + ", 100%, 70%)",
          f = Services.findOne({service: svc}) || Services.insert({
              type: "service",
              port: d,
              color: e,
              service: svc || "%E6%9C%AA%E7%9F%A5",
              count: count,
              display: '<span style="color: ' + e + '">&bigcirc;</span> ' + (svc || "%E6%9C%AA%E7%9F%A5")
            });
        ++f.count,
          Services.update(f),
          ["target", "source"].forEach(function (a, d) {
            var e, f = c[a];
            if (!(e = z.findOne({key: f.key, type: a}))) {
              ...
              e = z.insert({
                ...
                count: 0,
                r: 20
              }), v[datum.key + datum.type] = []
            }
            c[a].document = e, ++e.count;
            var i, j = f.country;
            if (!(i = B.findOne({$and: [{type: a}, {country: j}]}))) {
              var k = "<";
              k += 'img class="flag" src="img/flags/' + j + '.png"> ' + countryCodes[j], isInline && (k = countryCodes[j]), i = B.insert({
                type: a,
                country: j,
                countryName: countryCodes[j],
                display: k,
                count: 0
              })
            }
        ...
        var _source = c.source.document,
          _target = c.target.document,
        ...
        var l = datum.zerg ? {x: 20 * (Math.random() - .5), y: 20 * (Math.random() - .5)} : {x: 0, y: 0};
        n.append("circle")
          .classed("animating", !0)
          .attr({
            "cx": _source.lx + l.x,
        ...
        var o = (new Mover(ob, svgLayer),
          [{type: "source", data: C.data().slice(0, 10)}, {
            type: "service",
            data: E.data().slice(0, 10)
          }, {type: "target", data: D.data().slice(0, 10)}]);
        datum.zerg || a(o) && p.redraw(), k.embed || (o.push({
          type: "events",
    ...
    var l, m, n, o, p, q, r, s, t, u, v = {},
      ...
      attacks = new loki("attacks"),
      y = attacks.addCollection("timeline", {indices: ["source", "target", "service"]}),
      z = attacks.addCollection("anchors", {indices: ["key", "country"]}),
      Services = attacks.addCollection("services", {indices: ["port"]}),
      B = attacks.addCollection("countries", {indices: ["country", "type"]}),
      C = B.addDynamicView("source_countries");
    ...
    var E = Services.addDynamicView("sorted_services");
    ...
    {
      var countryCodes, J, K, M;
      d3.select("#preload")
    }

    isInline ?
      (document.getElementById("particleHiddenDiv").innerHTML += '<img src="assets/particle-inverse.png" id="particleHidden" style="visibility:hidden">', main(!1, json_config, nmap_services_1500, country_codes, json_layout, document.getElementById("particleHidden"))) :
      queue()
