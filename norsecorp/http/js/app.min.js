...
ipv.hash = {
  ...
  function reconnect () {
    var d = this;
    setTimeout(function () {
      c = ipv.stream.http(a, b), d.timeout *= 2
    }, this.timeout)
  }
...
Mover.prototype.startCurve = function () {
      ...
      if (datum, "O1" !== datum.country && "O1" !== datum.country2) {
        var c = {
          source: {
            ...
            locString: (datum.city ? datum.city + ", " : "") + datum.country
          },
          target: {
            key: (lat = datum.latitude2) + "_" + (lng = datum.longitude2),
            latLng: [+lat, +lng],
            country: datum.country2,
            locString: (datum.city2 ? datum.city2 + ", " : "") + datum.country2
          }
  ...
  }, x = new loki("attacks"), y = x.addCollection("timeline", {indices: ["source", "target", "service"]}), z = x.addCollection("anchors", {indices: ["key", "country"]}), A = x.addCollection("services", {indices: ["port"]}), B = x.addCollection("countries", {indices: ["country", "type"]}), C = B.addDynamicView("source_countries");
  ...
  isInline ? (document.getElementById("particleHiddenDiv").innerHTML += '<img src="assets/particle-inverse.png" id="particleHidden" style="visibility:hidden">', f(!1, json_config, nmap_services_1500, country_codes, json_layout, document.getElementById("particleHidden"))) : queue()
