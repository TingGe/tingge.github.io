...
// 单元数据格式化
  ...
  function (window, $) {
    ...
    function main(a, json_config, nmap_services_1500, country_codes, json_layout, particleHidden) {
      ...
      if (a)throw a;
      if (M = particleHidden, nmap_services_1500.forEach(function (a) {
        ...
        }), countryCodes = country_codes, g(json_config.map), !k.embed)if (J = ipv.ui().layout(json_layout), K = d3.select("body").append("div").attr("id", "ui").call(J), document.getElementById("ui").appendChild(document.getElementById("infobox")), isInline) {
    ...
    function renderCell(datum){
      if (datum, "O1" !== datum.country && "O1" !== datum.country2) {
        var c = {
          ...
          d = +datum.dport,
          e = "hsl(" + +d % 360 + ", 100%, 70%)",
          f = A.findOne({port: d}) || A.insert({
              type: "service",
              port: d,
              color: e,
              service: "未知",
              count: 0,
              display: '<span style="color: ' + e + '">&bigcirc;</span> 未知'
            });
        ++f.count,
          A.update(f),
    ...
    var l, m, n, o, p, q, r, s, t, u, v = {},
      ...
      A = x.addCollection("services", {indices: ["port"]}),
